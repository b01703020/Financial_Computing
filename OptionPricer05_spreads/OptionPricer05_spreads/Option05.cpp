...
namespace fre
{
    ...
    double Call::Payoff(double z) const { if(z>K) return z-K; return 0.0; }
    double Put::Payoff(double z) const { if(z<K) return K-z; return 0.0; }
    
    double bull_spread::Payoff(double z) const
    {
    ...
    double bear_spread::Payoff(double z) const
    {
        if(z>=K2) return 0;
