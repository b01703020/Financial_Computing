        ...
        BinLattice<double>& YTree)
    {
        ...
        for (int n = N - 1; n >= 0; n--)
        {
            for (int i = 0; i <= n; i++)
            {
                ...
                XPosition = (PriceTree.GetNode(n + 1, i + 1) - PriceTree.GetNode(n + 1, i)) / (Model.CalculateAssetPrice(n + 1, i + 1) - Model.CalculateAssetPrice(n + 1, i));
                YPosition = (PriceTree.GetNode(n + 1, i) - XPosition * Model.CalculateAssetPrice(n + 1, i)) / Model.GetR();
